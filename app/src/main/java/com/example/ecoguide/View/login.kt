                        ...
                        if (response.isSuccessful) {
                            ...
                            loginResponse?.let {
                                if (it.Token != null) {
                                    ...
                                    Snackbar.make(binding.root, "Connexion réussie : ${it.Token}", Snackbar.LENGTH_LONG).show()
                                    Log.d("LoginSuccess", "Token: ${it.Token}")
                                } else {
                                    // Gérez le cas où le token est null
                                    Snackbar.make(binding.root, "La connexion a réussi, mais le token est absent", Snackbar.LENGTH_LONG).show()
                                }
                            } ?: run {
                                // Gérez le cas où loginResponse est null
                                Snackbar.make(binding.root, "Réponse de connexion invalide", Snackbar.LENGTH_LONG).show()
                            }
                        } else {
                            // Gérez les autres cas de réponse non réussie
                            Snackbar.make(binding.root, "Échec de la connexion : ${response.errorBody()?.string()}", Snackbar.LENGTH_LONG).show()
